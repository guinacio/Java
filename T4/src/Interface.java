...
public class Interface
{
	...
	public void displayA(Aluno a, double c, int f, boolean progr)
	{
		String s = "";
		if(f==1)
			s = s + "\nO MELHOR ALUNO: \n";
		...
		while(nProv > 0)
		{
			s = s + "\nProva " + nProv + ":" + a.getNTeste((nProv-1));
			nProv--;
		}
	    ...
	    if(a.aprov(c))
	    {
	    	...
	    	s = s + "\nAluno Reprovado!";
	    if(progr)
	    	s = s + "\nO Aluno teve notas progressivas!";
	    else
	    	s = s + "\nO Aluno não teve notas progressivas.";
	...
	public void showAvPF(double s, int q)
	{
		JOptionPane.showMessageDialog(null,"A média da turma na prova final é: " + s/q);
	}
}